...
class CategoryManagerController {
  async create(req: Request, res: Response) {
    ...
    res.status(201).send(response("new category", result));
  }
  ...
  async get(req: Request, res: Response) {
    ...
    res.status(200).send(response("category details", result));
  }

  async getByUser(req: Request, res: Response) {
    const result = await CategoryManagerService.getByUser(
      parseInt(req.params.id)
    );
    res.status(200).send(response("categories by this user", result));
  }
